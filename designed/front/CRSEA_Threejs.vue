...
export default {
  ...
  methods: {
    ...
    async fetchData() {
      ...
      try {
        const response = await axios.get('/api/dashboard/c1')
      ...
      try {
        ...
        if (this.selectedTunnel) {
          ...
          response = await axios.put(
            `/api/dashboard/c1/tunnel/${this.selectedTunnel.id}/position`,
            { threeD_position: this.coordinateForm }
          )
          ...
          response = await axios.post(
            `/api/dashboard/c1/workfaces/${this.selectedWorkface.id}/translate`,
            { 
              new_reference_point: this.coordinateForm.reference_point
            }
          )
          
          if (response.data.status === 'success') {
            this.$message.success('工作面参照点坐标保存成功')
          }
          
          // 保存工作面旋转角度
          const rotateResp = await axios.post(
            `/api/dashboard/c1/workfaces/${this.selectedWorkface.id}/rotate`,
            { 
              rotation_angle: this.coordinateForm.rotation_angle
            }
          )
          if (rotateResp.data.status === 'success') {
            this.$message.success('工作面旋转角度保存成功')
          }
